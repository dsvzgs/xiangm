...
public class jiaocha extends CrossoverPipeline {
    /**
    ...
    // 🌟 计算子树哈希值（用于检测相似性）
//    private int computeSubtreeHash(GPNode node) {
//        return node.makeLispTree().hashCode();  // 基于 S 表达式的哈希计算
//    }
    // 子树相似性检测（避免无效交叉）
    private boolean areSubtreesIdentical(GPNode inner1, GPNode inner2) {
        String lispTree1 = inner1.makeLispTree();
        String lispTree2 = inner2.makeLispTree();
        return lispTree1.equals(lispTree2);
    }
    ...
    public boolean verifyPoints(final GPInitializer initializer,
        ...
        if (areSubtreesIdentical(inner1, inner2)) {
            return false; // 子树结构相同，避免交叉
        }
        // check for size
