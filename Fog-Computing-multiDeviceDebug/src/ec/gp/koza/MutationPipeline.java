...
// ✅ 获取规则类型
                    RuleType ruleType = RuleType.ROUTING;  // 默认 ROUTING
                    if (p1.rootParent() instanceof GPTree) {
                        GPTree tree = (GPTree) p1.rootParent();
                        GPIndividual individual = (GPIndividual) tree.owner;

                        // ✅ 判断 `tree` 在 `individual.trees` 中的位置
                        for (int index = 0; index < individual.trees.length; index++) {
                            if (individual.trees[index] == tree) {
                                ruleType = (index == 0) ? RuleType.SEQUENCING : RuleType.ROUTING;
                                break;
                            }
                        }
                    }
